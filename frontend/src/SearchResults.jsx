    ...
    <Container fluid className="mt-3">
            ...
            <Card.Body>
              <div className="top-bar d-flex align-items-center justify-content-between">
                <div className="air-logo d-flex align-items-center gap-3">
                  ...
                  <p className="m-0 fs-5">{cheapestFlight?.legs[0]?.carriers.marketing[0].name}</p>
                </div>
                <p className="m-0 fs-5">{cheapestFlight?.legs[0]?.segments[0]?.flightNumber}</p>
              </div>
              <div className="dep-arr d-flex justify-content-around mt-3">
                <div className="origin d-flex flex-column justify-content-center align-items-center gap-4">
                  ...
                  <Card.Text className="fs-5">{cheapestFlight?.legs[0]?.origin?.name}</Card.Text>
                </div>
                <div className="destination d-flex flex-column justify-content-center align-items-center gap-4">
                  ...
                  <Card.Text className="fs-5">{cheapestFlight?.legs[0]?.destination?.name}</Card.Text>
                </div>
              </div>
              ...
              <div className="date-time d-flex justify-content-around align-items-center">
                <Card.Text className="d-flex flex-column justify-content-center align-items-center fs-5 gap-2 m-0">
                  <p className="m-0">{formatDate(cheapestFlight?.legs[0]?.departure)}</p>
                  <p className="m-0">{formatTime(cheapestFlight?.legs[0]?.departure)}</p>
                </Card.Text>

                {/*cheapestFlight?.legs[1]?.origin?.city*/}
                {cheapestFlight?.legs[1] && (
                  <Card.Text className="d-flex flex-column justify-content-center align-items-center fs-5 gap-2 m-0">
                    <p className="m-0">{formatDate(cheapestFlight?.legs[1]?.departure)}</p>
                    <p className="m-0">{formatTime(cheapestFlight?.legs[1]?.departure)}</p>
                  </Card.Text>
                )}
              </div>
      ...
      <div className="Hotel mt-2 mb-4 border border-1 shadow-lg overflow-hidden">
        ...
        {!loadingHotels && hotels.length > 0 && (
          <Row className="hotel-row d-flex justify-content-between overflow-hidden">
            <Col className="img-container col-6 p-0">
              <img src={hotels[0]?.images[0]} alt="hotel-image" />
            </Col>
            <Col className="info-container col-6">
              <h2 className="text-center">{hotels[0]?.name}</h2>
              <div className="d-flex justify-content-between px-2">
                <div className="d-flex">
                  <p className="m-0 fs-5"> {hotels[0]?.stars || "Non classificato"}</p>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="rgb(247, 226, 2)" class="bi bi-star-fill" viewBox="0 0 16 16">
